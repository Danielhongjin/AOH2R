...
boss_aghanim_crystal_attack = class({})
...
LinkLuaModifier( "modifier_aghanim_crystal_attack_active", "abilities/bosses/boss_aghanim_crystal_attack", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_aghanim_crystal_attack_debuff", "abilities/bosses/boss_aghanim_crystal_attack", LUA_MODIFIER_MOTION_NONE )

----------------------------------------------------------------------------------------

function boss_aghanim_crystal_attack:Precache( context )
...
function boss_aghanim_crystal_attack:OnSpellStart()
	if IsServer() then
...
function boss_aghanim_crystal_attack:LaunchCrystals( vPos )
	...
	local fx = aoe_particle(self:GetCaster(), self.attack_time, vPos, self.explosion_radius, 2)
...
function boss_aghanim_crystal_attack:OnProjectileHit( hTarget, vLocation )
	if IsServer() then
		if hTarget ~= nil then
			return false
		end
