LinkLuaModifier("modifier_boss_phantom_lancer_counter", "abilities/bosses/boss_phantom_lancer_counter.lua", LUA_MODIFIER_MOTION_NONE)
...
require("lib/my")
require("lib/timers")
boss_phantom_lancer_counter = class({})


function boss_phantom_lancer_counter:OnSpellStart()
	...
	Timers:CreateTimer(
		delay, 
		function()
			caster:AddNewModifier(caster, self, "modifier_boss_phantom_lancer_counter", {duration = self:GetSpecialValueFor("duration")})
		end
	)
end

modifier_boss_phantom_lancer_counter = class({})

function modifier_boss_phantom_lancer_counter:IsPurgable()
	return true
end

function modifier_boss_phantom_lancer_counter:IsHidden()
	return false
end

function modifier_boss_phantom_lancer_counter:GetStatusEffectName()
    return "particles/custom/counter_colorwarp.vpcf"
end


function modifier_boss_phantom_lancer_counter:StatusEffectPriority()
    return 100
end

function modifier_boss_phantom_lancer_counter:DeclareFunctions()
    return {
...
if IsServer() then
	function modifier_boss_phantom_lancer_counter:OnCreated(keys)
	...
	function modifier_boss_phantom_lancer_counter:OnAttacked(keys)
		...
		if self.parent == victim and not self.has_procced then
			...
			local omnislash = self.parent:FindAbilityByName("boss_phantom_lancer_swift_slash")
			self.parent:SetCursorCastTarget(attacker)
			omnislash:OnSpellStart()
		end
	end
	
	function modifier_boss_phantom_lancer_counter:OnDestroy()
